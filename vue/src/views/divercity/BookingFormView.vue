<template>
  <div class="BookingFormView" v-if="space">
    <div class="BookingFormView__header">
      <PageTitle :title="$t('divercity.booking.title')" />
    ...
    <v-stepper v-if="!isDone" v-model="currentStep" class="BookingFormView__stepper" flat non-linear>
      ...
      <v-form @submit.prevent>
        ...
        <v-stepper-window-item :value="2">
            <div class="Form Form--booking">
                ...
                <div class="Form__fieldCtn">
                <label class="Form__label required">{{ $t('divercity.booking.fields.agenda') }}</label>
                <v-file-input
                    density="compact"
                    variant="outlined"
                    accept=".pdf"
                ...
                <div class="Form__fieldCtn">
                <label class="Form__label required">{{ $t('divercity.booking.fields.resourceDocument') }}</label>
                <v-file-input
                    density="compact"
                    variant="outlined"
                    accept=".pdf"
      ...
      <div class="BookingFormView__actions">
        ...
        <v-btn v-else color="main-red" :loading="isSubmitting" :disabled="hasAvailabilityConflict" @click="submitBooking">
          {{ $t('divercity.form.finish') }}
        </v-btn>
      </div>
    </v-stepper>
...
import type { BookingSubmission, EventActivityType, InformationSource } from '@/models/interfaces/divercity/Booking'
...
const { form, handleSubmit, isSubmitting, stepFields } = BookingFormService.getBookingForm({
...
const hasUnsavedChanges = computed(() => {
  return Object.values(form).some((field) => field.meta.dirty)
})
...
onMounted(async () => {
  ;[eventActivityTypes.value, informationSources.value] = await Promise.all([
    SpacesService.getEventActivityTypes(),
    SpacesService.getInformationSources()
  ])

  if (spacesStore.preselectedSlot) {
    ...
    spacesStore.preselectedSlot = null // consommé, on nettoie
  }
...
// Vérifie si le créneau saisi chevauche une réservation ou une période bloquée existante
const conflictingSlot = computed(() => {
  ...
  return (
    dayAvailability.value.find((slot) => {
      // chevauchement classique : startA < endB && startB < endA
